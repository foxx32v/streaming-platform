...
import { IChangePassword, IForgetPassword, ILogin, ILogout, IPagination, IRefresh, IRegistration, IResendVerification, IResetPassword, IResponse, IVerificationEmail, sessionDto, userDto, IRegisterResponse, ILoginResponse, IRefreshResponse, IGetAllUsersResponse, IVerifyTokenResponse, IValidateTokenResponse } from "../dto";
import { AxiosGet, AxiosPost, AxiosDelete, AxiosPut, AxiosPatch } from "./";

class AuthApi {
    async Register(body: IRegistration) {return AxiosPost<IResponse<IRegisterResponse>>(`${PATH_API.auth}/register`, body)}
    async Login(body: ILogin) {return AxiosPost<IResponse<ILoginResponse>>(`${PATH_API.auth}/login`, body)}
    async Logout() {return AxiosDelete<IResponse>(`${PATH_API.auth}/logout`, true)}
    async Refresh(body: IRefresh) {return AxiosPost<IResponse<IRefreshResponse>>(`${PATH_API.auth}/refresh`, body)}
    ...
    async GetAllUsers(params: IPagination) {return AxiosGet<IResponse<IGetAllUsersResponse>>(`${PATH_API.auth}/users?page=${params.page || 1}&limit=${params.limit || 10}`, true)}
    async GetUserById(userId: string) {return AxiosGet<IResponse<userDto>>(`${PATH_API.auth}/user/${userId}`, true)}
    ...
    async GetSessions() {return AxiosGet<IResponse<sessionDto[]>>(`${PATH_API.auth}/sessions`, true)}
